...
export function validatePhoneParam(
  req: Request,
  res: Response,
  next: NextFunction,
) {
...
export function validatePatchDoctor(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  const {error} = patchDoctorSchema.validate(req.body);
  if (error) return res.status(412).json({message: error.message});
  next();
}
...
export function validateDeviceBody(
  req: Request,
  res: Response,
  next: NextFunction,
) {
